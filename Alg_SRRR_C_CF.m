function [mu,A,B,D,obj] = Alg_SRRR_C_CF(theta,gamma,xi,MaxItr,N,P,R,Q,S,Y,X,Z) 
    ...
    while itr <= MaxItr
        ...
        for i = 1:Q
            ...
            c       = c + xi*bi_norm/(bi_norm+theta) - xi*d(i)*bi_norm;
        end
        First_Term  = eye(Q)-X*transpose(X)/Lambda_Max; % can be further simplified
        Second_Term = X*transpose(Y-mu*transpose(One)-D*Z)/Lambda_Max;
