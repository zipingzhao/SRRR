function [mu,A,B,D,obj] = Alg_SRRR_C_Cvx(theta,gamma,xi,MaxItr,N,P,R,Q,S,Y,X,Z) 
    ...
    while itr <= MaxItr
        ...
        cvx_begin quiet %sometimes not very accurate which can lead to ossilating
            variable B_cvx(Q,R)
            Reg_B_cvx = 0;
            for i = 1:Q
                Reg_B_cvx = Reg_B_cvx+xi*d(i)*norm(B_cvx(i,:));
            end 
            minimize(.5*sum(sum_square_abs(transpose(A)*(Y-mu*OneT-D*Z)-transpose(B_cvx)*X)));
            subject to
                Reg_B_cvx <= gamma-c
        cvx_end
        B = B_cvx;
